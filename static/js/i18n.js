(function () {
    ...
    const publicPhrases = {
        zh: {
            ...
            'What is New': 'v5.3 新特性',
            'Version 5.3 brings multi-timeframe consensus signals, outcome learning, and institutional liquidity/event/portfolio gates.': 'v5.3 带来了多周期共识信号、真实结果学习，以及机构级流动性、事件时段和组合风控门控。',
            'MTF Consensus Scanner': '多周期共识扫描器',
            'Emit one final long, short, or neutral decision per symbol. Higher timeframes carry more weight while lower timeframes refine entry timing.': '每个标的只输出一个最终做多、做空或空仓决策。高周期权重更高，低周期用于优化入场时机。',
            'Outcome Learning': '真实结果学习',
            'Closed scanner trades are labeled back into audit history so factor performance, win rate, and walk-forward score thresholds can learn from real results.': '已关闭的扫描器交易会回写到审计历史中，让因子表现、胜率和 walk-forward 分数阈值基于真实结果学习。',
            'Institutional Hard Gates': '机构级硬门控',
            'Liquidity depth, estimated slippage, funding/session blackout windows, and correlated portfolio exposure caps filter weak setups before AI review.': '在 AI 复核前，通过流动性深度、估算滑点、资金费率/时段黑名单和相关资产组合暴露上限过滤低质量机会。',
            'New in v5.3': 'v5.3 新功能',
            'Simple, Transparent Pricing': '简单透明的定价',
            'Start free. Upgrade when you need advanced AI models, multi-model voting, and institutional risk layers.': '免费开始。当你需要高级 AI 模型、多模型投票和机构级风控时再进行升级。',
            'Free': '免费版',
            'For personal hobby trading': '适合个人爱好交易',
            'Forever free': '永久免费',
            'Pro': '专业版',
            'For active traders': '适合活跃交易者',
            'per month': '每月',
            'Enterprise': '企业版',
            'For teams and funds': '适合团队与基金',
            'Contact us': '联系我们',
            'Start Free Trial': '开始免费试用',
            'Contact Sales': '联系销售',
            '1 Exchange connection': '1 个交易所连接',
            'Basic pre-filters': '基础预过滤器',
            'Community support': '社区支持',
            '3 Exchange connections': '3 个交易所连接',
            'Live + Paper trading': '实盘 + 模拟交易',
            'AI analysis (1 provider)': 'AI 分析（1 个提供商）',
            'Multi-TP & trailing stop': '多级止盈与移动止损',
            'Scanner access': '扫描器访问权限',
            'Email alerts': '邮件提醒',
            'Unlimited exchanges': '无限交易所连接',
            'Multi-model AI voting': '多模型 AI 投票',
            'DCA / Grid bots': '定投 / 网格机器人',
            'Custom risk profiles': '自定义风险偏好',
            'Priority Telegram support': '优先 Telegram 支持',
            'White-label options': '白标选项',
            'Frequently Asked Questions': '常见问题',
            'Everything you need to know before deploying your first strategy.': '在部署你的第一个策略之前需要了解的一切。',
            'Is QuantPilot free to use?': 'QuantPilot 是免费的吗？',
            'Yes. The core engine is open-source under the MIT license. You can self-host for free. We also offer managed cloud plans with premium AI providers and priority support.': '是的。核心引擎基于 MIT 许可证开源。你可以免费自托管。我们还提供包含高级 AI 提供商和优先支持的托管云服务。',
            'Do I need coding skills?': '我需要编程技能吗？',
            'No. QuantPilot is designed as a plug-and-play bridge. Configure your exchange API keys, set risk limits in the web UI, and paste the webhook URL into TradingView. No strategy coding required.': '不需要。QuantPilot 被设计为即插即用的桥梁。配置交易所 API 密钥，在 Web 界面设置风险限制，然后将 Webhook URL 粘贴到 TradingView 中即可。不需要编写策略代码。',
            'Which exchanges are supported?': '支持哪些交易所？',
            'Binance, OKX, Bybit, Bitget, Gate.io, and Coinbase are supported out of the box. We use CCXT under the hood, so new exchanges can be added quickly.': 'Binance、OKX、Bybit、Bitget、Gate.io 和 Coinbase 开箱即用。我们在底层使用 CCXT，因此可以快速添加新交易所。',
            'How does the AI analysis work?': 'AI 分析如何工作？',
            'When a signal arrives, QuantPilot sends market context (price, volume, trend, support/resistance) to your chosen AI provider. The model returns a confidence score, risk assessment, and recommended leverage. You set the minimum confidence threshold in Settings.': '当信号到达时，QuantPilot 将市场上下文（价格、成交量、趋势、支撑/阻力）发送给你选择的 AI 提供商。模型返回置信度评分、风险评估和推荐杠杆。你可以在设置中设定最低置信度阈值。',
            'What happens if my internet or the exchange API goes down?': '如果我的网络或交易所 API 中断怎么办？',
            'QuantPilot includes ghost position detection, exchange reconciliation, and automatic retry logic. If an order fails, the system logs the reason and can alert you via Telegram. Live positions are reconciled every few minutes to detect mismatches.': 'QuantPilot 包含幽灵持仓检测、交易所对账和自动重试逻辑。如果订单失败，系统会记录原因并通过 Telegram 提醒你。实盘持仓每隔几分钟进行对账以检测不匹配。'
        },
        ja: {
            'Platform': 'プラットフォーム',
            'Features': '機能',
