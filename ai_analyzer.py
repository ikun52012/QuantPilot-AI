...
AUTO_SCANNER_PROMPT = """
## Auto Scanner Signal Defense

For signals generated by AI_Auto_Scanner:
1. Treat the scanner as a candidate generator, not as confirmation.
2. Require a precise entry, structural stop loss, and at least one realistic take-profit level.
3. Require confidence at or above the configured scanner minimum before execute/modify; otherwise return reject or hold.
4. Reject late entries, unclear invalidation, poor liquidity, stale data, thin risk/reward, and setups that are only indicator-based without price-location support.
5. Prefer modify when waiting for the FVG/OB midpoint gives better risk/reward than entering immediately.
"""
...
def _analysis_config_signature(user_settings: dict | None = None) -> str:
    ...
    payload = {
        ...
        "scanner": (user_settings or {}).get("_scanner_context") or {},
    }
...
def _get_effective_system_prompt(user_settings: dict | None = None) -> str:
    ...
    base += "\n" + SMC_FVG_PROMPT
    if isinstance(user_settings, dict) and user_settings.get("_scanner_context"):
        base += "\n" + AUTO_SCANNER_PROMPT
        scanner_min_confidence = (user_settings.get("_scanner_context") or {}).get("min_confidence", 0.70)
        base += f"\nConfigured scanner minimum confidence: {float(scanner_min_confidence):.2f}."
...
- Trailing Step %: {ts_config.trailing_step_pct}%"""
    ...
    scanner_section = ""
    if signal.strategy == "AI_Auto_Scanner" or scanner_context:
...
- Required Minimum Confidence: {scanner_min_confidence}
- The signal was generated by an automated market scan, not by a human-curated TradingView alert.
- Only recommend execute/modify when the setup has a clear structural edge, valid entry, valid invalidation, and realistic TP path.
- If entry, SL, TP, direction, or risk/reward cannot be stated precisely, return reject or hold.
- Scanner Payload: {scanner_text}
"""
